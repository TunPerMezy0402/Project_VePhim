...
<a href="{{ route('admin.cinemas.rooms.index', $cinema->id) }}" class="btn btn-danger btn-sm mb-3">
    <i class="fas fa-arrow-left me-1"></i> Quay lại
</a>

<div class="page-header">
    <h1><i class="fas fa-edit me-3"></i>Chỉnh sửa phòng chiếu</h1>
    <div class="subtitle">Cập nhật cấu hình ghế ngồi cho phòng {{ $room->name }}</div>
</div>

<form id="room-form" method="POST" action="{{ route('admin.cinemas.rooms.update', ['cinema' => $cinema->id, 'room' => $room->id]) }}">
    @csrf
    @method('PUT')
    <div class="row">
        <div class="col-lg-8">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card vip">
                    <h6>Ghế số ghế</h6>
                    <h3 id="vip-count">{{ $room->seats->count() }}</h3>
                </div>
...
<script>
    ...
    loadExistingData() {
        ...
        const roomData = @json($room->toArray());
        const seatRows = @json($room->seatRows->toArray());
        const seats = @json($room->seats->toArray());
        
        if (seatRows.length > 0) {
            this.rows = seatRows.length;
            this.seatsPerRow = Array(this.rows);
            
            // Load row configuration
            seatRows.forEach((row, index) => {
                const rowIndex = row.row_label.charCodeAt(0) - 65; // Convert A,B,C... to 0,1,2...
                this.seatsPerRow[rowIndex] = row.total_seats;
        ...
        if (seats.length > 0) {
            seats.forEach(seat => {
                ...
                const seatNumber = parseInt(seat.seat_number.slice(1)) - 1; // Convert A01 to row=0, col=0
                ...
                // Update pricing based on existing seat prices
                if (seat.seat_chair === 'single' && !this.vipRows.has(rowIndex)) {
                    document.getElementById('normal-price').value = seat.price || 50000;
                } else if (seat.seat_chair === 'couple' && !this.vipRows.has(rowIndex)) {
                    document.getElementById('couple-price').value = seat.price || 120000;
                } else if (seat.seat_chair === 'single' && this.vipRows.has(rowIndex)) {
                    document.getElementById('vip-price').value = seat.price || 80000;
                } else if (seat.seat_chair === 'couple' && this.vipRows.has(rowIndex)) {
                    document.getElementById('vip-couple-price').value = seat.price || 200000;
                }
            });
        }
    }
    ...
    update() {
        ...
        this.updateFormData();
        document.getElementById('seats-per-row-display').textContent = this.seatsPerRow[0] || 10;
    }
    ...
    async handleSubmit() {
        ...
        try {
            ...
            if (response.ok) {
                this.notify('Cập nhật cấu hình thành công!', 'success');
                setTimeout(() => {
                    window.location.href = result.redirect || '{{ route("admin.cinemas.rooms.index", $cinema->id) }}';
                }, 1500);
            } else {
                throw new Error(result.message || 'Server error');
            }
        } catch (error) {
            this.notify('Có lỗi xảy ra khi cập nhật: ' + error.message, 'error');
